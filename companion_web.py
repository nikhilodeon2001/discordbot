...
<style>
  :root {
    ...
    --blue:#1ea0ff; --blue-600:#0b84e6; --red:#ff2a2a;
    --bg:#0b0e14; --bg2:#121722; --fg:#eef2f8; --muted:rgba(238,242,248,.58);
    --card:rgba(255,255,255,.05); --line:rgba(255,255,255,.10);
    --play:url(__PLAY_LOGO__); --okra:url(__OKRA_LOGO__);
  }
  @media (prefers-color-scheme: light) {
    :root { --bg:#eef2f8; --bg2:#f7f9fc; --fg:#111621; --muted:rgba(17,22,33,.55);
            --card:#ffffff; --line:rgba(17,22,33,.10); }
  }
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  body { margin:0; min-height:100vh; color:var(--fg);
    font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Inter,sans-serif;
    background:radial-gradient(1100px 560px at 50% -12%, rgba(30,160,255,.14), transparent 60%),
  ...
  .timer { flex:none; font-variant-numeric:tabular-nums; font-weight:800; font-size:.74rem;
    padding:4px 10px; border-radius:999px; background:rgba(30,160,255,.16); color:var(--blue); }
  .timer.low { background:rgba(255,42,42,.16); color:var(--red); }
  ...
  button.primary { width:100%; margin-top:12px; padding:15px; font-size:1.05rem; font-weight:700;
    ...
    background:linear-gradient(180deg,var(--blue),var(--blue-600)); box-shadow:0 6px 16px rgba(30,160,255,.30); }
...
function render(state) {
  ...
  if (state.phase === 'revealed') {
    ...
    app.innerHTML = '<div class="cat">' + esc(state.category || '') + '</div>' +
      ...
      imgHtml(state) +
      answerLine + mine +
      scoreboardHtml(state) + legendHtml(state);
  ...
  app.innerHTML = '<div class="qhead"><span class="cat">' + esc(state.category || '') + '</span>' +
      ...
      '<div id="status" class="status"></div>' + legendHtml(state);
